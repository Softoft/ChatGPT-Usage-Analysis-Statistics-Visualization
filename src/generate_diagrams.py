...
def plot_scatter_with_moving_avg(csv_path, png_path, html_path):
    ...
    plt.figure(figsize=(10, 6))
    plt.scatter(df["date"], df["number_of_messages"], color="blue", label="Number of Messages", s=1)
    plt.plot(df["date"], df["moving_avg_7"], color="gray", label="7-Day Moving Average")
    plt.xlabel("Date")
    plt.ylabel("Number of Messages")
